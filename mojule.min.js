...
allOfType:function(b){return i(a).all(function(a){return typeof a===b})},firstDefined:function(){return i(a).first(function(a){return void 0!==a})},anyDefined:function(){return i(a).any(function(a){return void 0!==a})},allDefined:function(){return i(a).all(function(a){return void 0!==a})},elementsEqual:function(b){if(a.length!==b.length)return!1;for(var d=0;d<b.length;d++){if(i(a[d]).elementsEqual)if(i(a[d]).elementsEqual(b[d]))continue;else return!1;if(void 0!==a[d].equal&&"function"===typeof a[d].equals){if(!a[d].equals(b[d]))return!1}else if(a[d]!==
...
b,c,d){for(var l=function(c){b[c]=function(){for(var l=0;l<d.length;l++){var g=d[l];b[g]=a[c].apply(a,[b[g]])}return b}},g=0;g<c.length;g++)l(c[g])};var q=mojule.Extensions;mojule.Mapper=function(a){var b=this,c={undefinedValue:{predicate:function(a){return void 0===a||null===a},map:function(a,b){return b}},toArray:function(){var a=[],b;for(b in this)this.hasOwnProperty(b)&&"function"!==typeof this[b]&&a.push({name:b,predicate:this[b].predicate,map:this[b].map});return a}};this.map=function(a,b){var g=
q(b).toValue(),e=q(c.toArray()).first(function(a){return a.predicate(g)});return!e?void 0:e.map(g,a)};this.hasMapping=function(a){return q(c.toArray()).any(function(b){return b.predicate(a)})};this.getMappingName=function(a){var b=q(c.toArray()).first(function(b){return b.predicate(a)});if(void 0!==b)return b.name};this.addMapping=function(a,b){c[a]=b};this.addMappings=function(a){q(a).each(function(a,d){"function"!==typeof a&&b.addMapping(d,{predicate:a.predicate,map:a.map})})};this.removeMapping=
function(a){void 0!==c[a]&&delete c[a]};void 0!==a&&(a instanceof Array?q(a).each(function(a){b.addMappings(a)}):b.addMappings(a))};var s=function(a){return void 0!==a.value&&0<a.value.length?a.setter():a.getter()},t=function(a,b,c){a.clone=function(){return new b(a)};a.toString=function(){return f(a).getPropertyValues(b.properties).join(",")};a.equals=function(){var d=f(arguments).toObject(b);return f(a).equals(d,b.properties)};a.almostEquals=function(){var d=f(arguments).toObject(b);return f(a).almostEquals(d,
b.properties)};f(b.properties).each(function(b){a[b]=0});b.mapper.map(a,c);f.addMath(a,b.properties)},u=function(a,b){var c={args:{predicate:function(a){return a instanceof Array&&1<a.length&&f(a).allOfType("number")},map:function(b,c){f(a.properties).each(function(a,e){c[a]=b.length>e?parseFloat(b[e]):c[a]});return c}},str:{predicate:function(a){return"string"===typeof a&&0<a.match(x).length},map:function(b,c){var g=b.match(x);f(a.properties).each(function(a,b){c[a]=g.length>b?parseFloat(g[b]):c[a]});
...
k=function(b,c){f(a.properties).each(function(a){var d=[b[a]];void 0!==m&&void 0!==m[a]&&d.push(b[m[a]]);d.push(c[a]);c[a]=parseFloat(f(d).firstDefined())});return c}}else k=f([b.properties,b.map]).allDefined()?function(a,c){b.map(a,c);return c}:void 0!==b.copy?function(a,c){f(b.copy).each(function(b){c[b]=a[b]});return c}:function(a,b){return b};c[e]={predicate:g,map:k}});return c},y=function(a,b,c,d,e){var g=Math.min(b,d),f=Math.min(c,e);a.left=g;a.top=f;a.width=Math.max(b,d)-g;a.height=Math.max(c,
e)-f},f=mojule.Extensions,x=/[+\-]?(?:\d+\.?\d*|\d*\.?\d+)/g;mojule.Point=function(){var a=this;t(a,mojule.Point,arguments);this.unitVector=function(){return new Point(a.x/a.length(),a.y/a.length())};this.length=function(){return s({value:arguments,setter:function(){var b=a.unitVector(),c=f(this.value).toValue();a.x=b.x*c;a.y=b.y*c;return a},getter:function(){return Math.sqrt(a.x*a.x+a.y*a.y)}})};this.scale=function(b){a.x*=b;a.y*=b;return a};this.translate=function(){var b=f(arguments).toObject(mojule.Point);
...
f(arguments).toObject(mojule.Point);return new mojule.Line(b,a)};this.neighbours=function(){return[new mojule.Point(a.x,a.y-1),new mojule.Point(a.x,a.y+1),new mojule.Point(a.x-1,a.y),new mojule.Point(a.x+1,a.y)]};this.wrap=function(){var b=new mojule.Size(arguments);a.x=f(a.x).wrap(b.width);a.y=f(a.y).wrap(b.height);return a}};mojule.Point.properties=["x","y"];mojule.Point.mapper=new mojule.Mapper(u(mojule.Point,{line:{properties:["x1","y1","x2","y2"],map:function(a,b){b.x=Math.max(a.x1,a.x2)-Math.min(a.x1,
a.x2);b.y=Math.max(a.y1,a.y2)-Math.min(a.y1,a.y2)}},rectangleOrSize:{aliases:{x:"width",y:"height"}},rectangle2:{properties:["left","top","right","bottom"],map:function(a,b){b.x=Math.max(a.left,a.right)-Math.min(a.left,a.right);b.y=Math.max(a.top,a.bottom)-Math.min(a.top,a.bottom)}},leftTop:{aliases:{x:"left",y:"top"}}}));mojule.Line=function(){var a=this;t(a,mojule.Line,arguments);this.start=function(){return s({value:arguments,setter:function(){var b=f(this.value).toObject(mojule.Point);a.x1=b.x;
a.y1=b.y;return a},getter:function(){return new mojule.Point(a.x1,a.y1)}})};this.end=function(){return s({value:arguments,setter:function(){var b=f(this.value).toObject(mojule.Point);a.x2=b.x;a.y2=b.y;return a},getter:function(){return new mojule.Point(a.x2,a.y2)}})};this.verticals=function(){return[a.x1,a.x2]};this.horizontals=function(){return[a.y1,a.y2]};this.points=function(){return[a.start(),a.end()]};this.bresenham=function(){for(var b=a.clone().round(),c=f(b.end().x).difference(b.start().x),
...
"y1","x2","y2"];mojule.Line.mapper=new mojule.Mapper([u(mojule.Line,{point:{aliases:{x2:"x",y2:"y"}},sizeOrRectangle:{properties:["width","height"],map:function(a,b){var c=void 0!==a.x?a.x:0,d=void 0!==a.y?a.y:0;b.x1=c;b.y1=d;b.x2=c+a.width;b.y2=d+a.height}}}),{points:{predicate:function(a){return a instanceof Array&&1<a.length&&f([a[0].x,a[0].y]).anyDefined()&&f([a[1].x,a[1].y]).anyDefined()},map:function(a,b){b.x1=parseFloat(a[0].x);b.y1=parseFloat(a[0].y);b.x2=parseFloat(a[1].x);b.y2=parseFloat(a[1].y);
return b}}}]);mojule.Size=function(){t(this,mojule.Size,arguments)};mojule.Size.properties=["width","height"];mojule.Size.mapper=new mojule.Mapper([u(mojule.Size,{point:{aliases:{width:"x",height:"y"}},line:{properties:["x1","y1","x2","y2"],map:function(a,b){b.width=Math.max(a.x1,a.x2)-Math.min(a.x1,a.x2);b.height=Math.max(a.y1,a.y2)-Math.min(a.y1,a.y2)}},rectangle:{copy:["width","height"]},rectangle2:{properties:["left","right","top","bottom"],map:function(a,b){b.width=Math.max(a.left,a.right)-Math.min(a.left,
a.right);b.height=Math.max(a.top,a.bottom)-Math.min(a.top,a.bottom)}}})]);mojule.Rectangle=function(){var a=this;t(a,mojule.Rectangle,arguments);this.right=function(){return a.left+a.width};this.bottom=function(){return a.top+a.height};this.inBounds=function(){var b=new mojule.Point(f(arguments).toValue());return b.x>=a.left&&b.x<=a.right()&&b.y>=top&&b.y<=a.bottom()}};mojule.Rectangle.properties=["left","top","width","height"];mojule.Rectangle.mapper=new mojule.Mapper([u(mojule.Rectangle,{rectangle3:{properties:["x",
"y","width","height"],map:function(a,b){b.width=a.width;b.height=a.height}},point:{aliases:{left:"x",top:"y"}},line:{properties:["x1","y1","x2","y2"],map:function(a,b){y(b,a.x1,a.y1,a.x2,a.y2)}},size:{copy:["width","height"]},rectangle2:{properties:["left","right","top","bottom"],map:function(a,b){y(b,a.left,a.top,a.right,a.bottom)}}})]);mojule.Grid=function(){function a(a,f,h){c[f*g.width+a]=h;void 0===d[f]&&(d[f]=[]);d[f][a]=h;void 0===e[a]&&(e[a]=[]);e[a][f]=h;return b}var b=this,c=[],d=[],e=[],
...
f(c).elementsEqual(a.cells())};this.bounds=function(){return new Rectangle(g)};this.rows=function(){return d.slice(0)};this.columns=function(){return e.slice(0)};var j=f(arguments).toValue();null!==j&&f([j.width,j.height,j.data]).allDefined()?(g=new mojule.Size(j.width,j.height),c=f(j.data).chunk(4)):(mojule.Size.mapper.map(g,arguments),g.ceil().abs())};var n=function(a){return void 0!==a.value?a.setter():a.getter()},o=function(a){return 0.299*a.red+0.587*a.green+0.114*a.blue},z=function(a){var b=
a.red/255,c=a.green/255,a=a.blue/255,d=Math.max(b,c,a),e=Math.min(b,c,a),g,f=(d+e)/2,k=d-e;if(d===e)g=e=0;else{e=0.5<f?k/(2-d-e):k/(d+e);switch(d){case b:g=(c-a)/k+(c<a?6:0);break;case c:g=(a-b)/k+2;break;case a:g=(b-c)/k+4}g/=6}return{hue:360*g,saturation:100*e,lightness:100*f}},A=function(a){"#"===a.substr(0,1)&&(a=a.substr(1));return 3===a.length?{red:parseInt(a.substr(0,1)+a.substr(0,1),16),green:parseInt(a.substr(1,1)+a.substr(1,1),16),blue:parseInt(a.substr(2,1)+a.substr(2,1),16)}:{red:parseInt(a.substr(0,
2),16),green:parseInt(a.substr(2,2),16),blue:parseInt(a.substr(4,2),16)}},w=function(a){a=a.toString(16);return 2>a.length?"0"+a:a},B=function(a,b){return{hue:e(a.hue*e([b.hue,b.h,1]).firstDefined()).wrap(360),saturation:e(a.saturation*e([b.saturation,b.s,1]).firstDefined()).clamp(0,100),lightness:e(a.lightness*e([b.lightness,b.l,1]).firstDefined()).clamp(0,100),alpha:e(a.alpha*e([b.alpha,b.a,1]).firstDefined()).clamp(0,1)}},C=function(a,b){return{hue:e(a.hue+e([b.hue,b.h,0]).firstDefined()).wrap(360),
...
this.value.r,c,0]).firstDefined()).clamp(0,255);d=e(e([this.value.green,this.value.g,d,0]).firstDefined()).clamp(0,255);f=e(e([this.value.blue,this.value.b,f,0]).firstDefined()).clamp(0,255);m=e(e([this.value.alpha,this.value.a,m,1]).firstDefined()).clamp(0,1);var b=z(a());g=b.hue;j=b.saturation;k=b.lightness;return h},getter:a})};this.hsl=this.hsla=function(a){return n({value:a,setter:function(){g=e(e([this.value.hue,this.value.h,g,360]).firstDefined()).wrap(360);j=e(e([this.value.saturation,this.value.s,
j,0]).firstDefined()).clamp(0,100);k=e(e([this.value.lightness,this.value.l,k,0]).firstDefined()).clamp(0,100);m=e(e([this.value.alpha,this.value.a,m,1]).firstDefined()).clamp(0,1);var a=b(),r=a.hue/360,i=a.saturation/100,p=a.lightness/100,a=function(a,b,c){0>c&&(c+=1);1<c&&(c-=1);return c<1/6?a+6*(b-a)*c:0.5>c?b:c<2/3?a+6*(b-a)*(2/3-c):a},n=0.5>p?p*(1+i):p+i-p*i,o=2*p-n;0===i?i=p=r=p:(i=a(o,n,r+1/3),p=a(o,n,r),r=a(o,n,r-1/3));c=255*i;d=255*p;f=255*r;return h},getter:b})};this.bytes=function(b){return n({value:b,
setter:function(){if(b instanceof Array){c=e(e([this.value[0],c]).firstDefined()).clamp(0,255);d=e(e([this.value[1],d]).firstDefined()).clamp(0,255);f=e(e([this.value[2],f]).firstDefined()).clamp(0,255);void 0!==this.value[3]&&(m=parseFloat((this.value[3]/255).toFixed(2)));var v=z(a());g=v.hue;j=v.saturation;k=v.lightness}return h},getter:function(){return[c,d,f,Math.round(255*m)]}})};this.toString=this.css=function(){return 1===parseFloat(m)?h.hex():a().css};this.hex=function(b){return n({value:b,
setter:function(){return h.rgba(A(this.value))},getter:function(){var b=a(),c=w(Math.round(b.red)),d=w(Math.round(b.green)),b=w(Math.round(b.blue));return"#"+(c.charAt(0)===c.charAt(1)&&d.charAt(0)===d.charAt(1)&&b.charAt(0)===b.charAt(1)?c.charAt(0)+d.charAt(0)+b.charAt(0):c+d+b)}})};this.brightness=function(){return o(a())};this.brightnessDifference=function(a){var b=h.rgba(),a=(new mojule.Color(a)).rgba();return Math.abs(o(b)-o(a))};this.colorDifference=function(a){var b=h.rgba(),c=(new mojule.Color(a)).rgba(),
a=Math.max(b.red,c.red),d=Math.min(b.red,c.red),e=Math.max(b.green,c.green),f=Math.min(b.green,c.green),g=Math.max(b.blue,c.blue),b=Math.min(b.blue,c.blue);return a-d+(e-f)+(g-b)};this.hasEnoughContrast=function(a,b,c){c=void 0!==c?c:125;return h.colorDifference(a)>=(void 0!==b?b:500)&&h.brightnessDifference(a)>=c};this.multiplyHsla=function(a){return h.hsla(B(h.hsla(),a))};this.sumHsla=function(a){return h.hsla(C(h.hsla(),a))};this.clone=function(){return new mojule.Color(h.hsla())};this.getContrastingTone=
function(){var a=mojule.Color,b;b=h.rgba();var c={red:255,green:255,blue:255},d={red:0,green:0,blue:0},e=Math.abs(o(b)-o(c));b=Math.abs(o(b)-o(d))-30>e?d:c;return new a(b)};this.equals=function(){var a=new mojule.Color(e(arguments).toValue());return e([this.red(),this.green(),this.blue(),this.alpha()]).elementsEqual([a.red(),a.green(),a.blue(),a.alpha()])};mojule.Color.mapper.map(h,arguments)};mojule.Color.regexes={cssHex:/^#(?:[0-9a-f]{3,6})\b$/i,cssRgb:/^rgb\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)$/i,
