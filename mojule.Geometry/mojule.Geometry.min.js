...
b){var d={args:{predicate:function(a){return a instanceof Array&&1<a.length&&c(a).allOfType("number")},map:function(b,d){c(a.properties).each(function(a,c){d[a]=b.length>c?parseFloat(b[c]):d[a]});return d}},str:{predicate:function(a){return"string"===typeof a&&0<a.match(p).length},map:function(b,d){var e=b.match(p);c(a.properties).each(function(a,b){d[a]=e.length>b?parseFloat(e[b]):d[a]});return d}}};c(b).each(function(b,j){var e;if(void 0!==b.aliases){var g=b.aliases;e=function(b){var d=[];c(a.properties).each(function(a){d.push(b[a]);
...
return d}}else i=c([b.properties,b.map]).allDefined()?function(a,d){b.map(a,d);return d}:void 0!==b.copy?function(a,d){c(b.copy).each(function(b){d[b]=a[b]});return d}:function(a,b){return b};d[j]={predicate:e,map:i}});return d}function q(a,b,d,c,j){var e=Math.min(b,c),g=Math.min(d,j);a.left=e;a.top=g;a.width=Math.max(b,c)-e;a.height=Math.max(d,j)-g}var c=mojule.Extensions,p=/[+\-]?(?:\d+\.?\d*|\d*\.?\d+)/g;mojule.Point=function(){var a=this;m(a,mojule.Point,arguments);this.unitVector=function(){return new Point(a.x/
