...
return c}}else i=e([d.properties,d.map]).allDefined()?function(a,c){d.map(a,c);return c}:void 0!==d.copy?function(a,c){e(d.copy).each(function(d){c[d]=a[d]});return c}:function(a,d){return d};c[b]={predicate:f,map:i}});return c}function r(a,b,c,d,p){var f=Math.min(b,d),e=Math.min(c,p);a.left=f;a.top=e;a.width=Math.max(b,d)-f;a.height=Math.max(c,p)-e}var g=mojule.Extensions=function(a){var b={Number:{clamp:function(c,d){return a<c?c:a>d?d:a},wrap:function(c){return(a%c+c)%c},toRadians:function(){return g(a).wrap(360)/
...
a)};this.neighbours=function(){return[new mojule.Point(a.x,a.y-1),new mojule.Point(a.x,a.y+1),new mojule.Point(a.x-1,a.y),new mojule.Point(a.x+1,a.y)]};this.wrap=function(){var b=new mojule.Size(arguments);a.x=e(a.x).wrap(b.width);a.y=e(a.y).wrap(b.height);return a}};mojule.Point.properties=["x","y"];mojule.Point.mapper=new mojule.Mapper(o(mojule.Point,{line:{properties:["x1","y1","x2","y2"],map:function(a,b){b.x=Math.max(a.x1,a.x2)-Math.min(a.x1,a.x2);b.y=Math.max(a.y1,a.y2)-Math.min(a.y1,a.y2)}},
...
["width","height"];mojule.Size.mapper=new mojule.Mapper([o(mojule.Size,{point:{aliases:{width:"x",height:"y"}},line:{properties:["x1","y1","x2","y2"],map:function(a,b){b.width=Math.max(a.x1,a.x2)-Math.min(a.x1,a.x2);b.height=Math.max(a.y1,a.y2)-Math.min(a.y1,a.y2)}},rectangle:{copy:["width","height"]},rectangle2:{properties:["left","right","top","bottom"],map:function(a,b){b.width=Math.max(a.left,a.right)-Math.min(a.left,a.right);b.height=Math.max(a.top,a.bottom)-Math.min(a.top,a.bottom)}}})]);mojule.Rectangle=
function(){var a=this;n(a,mojule.Rectangle,arguments);this.right=function(){return a.left+a.width};this.bottom=function(){return a.top+a.height};this.inBounds=function(){var b=new mojule.Point(e(arguments).toValue());return b.x>=a.left&&b.x<=a.right()&&b.y>=top&&b.y<=a.bottom()}};mojule.Rectangle.properties=["left","top","width","height"];mojule.Rectangle.mapper=new mojule.Mapper([o(mojule.Rectangle,{rectangle3:{properties:["x","y","width","height"],map:function(a,b){b.width=a.width;b.height=a.height}},
point:{aliases:{left:"x",top:"y"}},line:{properties:["x1","y1","x2","y2"],map:function(a,b){r(b,a.x1,a.y1,a.x2,a.y2)}},size:{copy:["width","height"]},rectangle2:{properties:["left","right","top","bottom"],map:function(a,b){r(b,a.left,a.top,a.right,a.bottom)}}})]);mojule.Grid=function(){function a(a,e,j){c[e*f.width+a]=j;void 0===d[e]&&(d[e]=[]);d[e][a]=j;void 0===g[a]&&(g[a]=[]);g[a][e]=j;return b}var b=this,c=[],d=[],g=[],f=new mojule.Size;this.get=function(){if(1<arguments.length&&"number"===typeof arguments[0]&&
